...
roseDGenerator="$ROSE_INSTALL_PATH/dotGenerator"
...
$roseDGenerator -c /tmp/test$mypid.$suffix -rose:output /tmp/rose_test$mypid.$suffix &>/tmp/test$mypid.$suffix.output

dot test$mypid.$suffix.dot -Tpng -o /tmp/test$mypid.png
dot test$mypid.$suffix.dot -Tpdf -o /tmp/test$mypid.pdf
#dot test$mypid.$suffix.dot -Tps2 -o /tmp/test$mypid.ps2
if [ $? -eq 0 ]
then
...
$roseDGenerator --version &>/tmp/rose$mypid.version
display_code "/tmp/rose$mypid.version"
echo "<p>"
echo "<form name=\"opennew\">"
echo "<select name=\"formats\" id=\"formats\">"
echo "<option value=\"\" selected>Select Format</option>"
echo "<option value=\"/imgs/test$mypid.png\">.PNG</option>"
echo "<option value=\"/imgs/test$mypid.pdf\">.PDF</option>"
#echo "<option value=\"/imgs/test$mypid.ps2\">.PS2</Option>"
echo "</select>"
echo "<input type=\"button\" name=\"Submit\" value=\"Go!\" onClick=\"window.open(formats.value)\"></form>"
